function Show-EpubController($requestObject) {
    Write-Host "epub controller"
    Show-Context

    #  Create model
     $model = @{
         category = "epub"
         items = Get-EpubTableOfContents -EpubFilePath $requestObject.ContextPath | ForEach-Object {
...
function Get-EpubTableOfContents {
    param (
        [string]$EpubFilePath
    )
